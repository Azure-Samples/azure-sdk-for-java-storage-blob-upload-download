...
public class quickstart {
    ...
    static void uploadFile(BlockBlobURL blob, File sourceFile){
		...
		try {
	    	...
	    	.doFinally(() -> {
	    		...
	    		System.out.println(">> Success : " + success.statusCode() + ", Etag: " + success.headers().eTag());
	    	}, error -> {
    ...
    static void listBlobs(ContainerURL containerURL){
    	...
    	// Each ContainerURL.listBlobs call return up to maxResults (maxResults=100 passed into ListBlobOptions below). 
    	// To list all Blobs, we are creating a helper static method called listAllBlobs
    	// This method keeps track of nextMarker, and repeat listBlobs call until there is no more nextMarker
   ...
   static void getBlob(BlockBlobURL blobURL){
	   ...
	   // Get the blob
	   // Since the blob is small, we'll read into memory asynchronously
    ...
    public static void main(String[] args){
		...
		try {
			...
			//int successCode = containerURL.create(null, null).blockingGet().statusCode();
			//System.out.println("Container creation succeeded with HTTP Status code: " + successCode );
